	...
	for _, formula := range formulas {
		...
		for rowIdx := dataStartRow; rowIdx < len(table); rowIdx++ {
			...
			for colIdx := 0; colIdx < len(row); colIdx++ {
				...
				evaluableExpr = rangeRefRe.ReplaceAllStringFunc(evaluableExpr, func(rangeRef string) string {
					matches := rangeRefRe.FindStringSubmatch(rangeRef)
					if matches == nil {
						return rangeRef
					}
				...
				if evaluableExpr == "" {
					...
					if num, err := strconv.ParseFloat(evaluableExpr, 64); err == nil {
						...
						// Register vsum and other functions
						vsumFunc := expr.Function(
							"vsum",
						...
						program, err := expr.Compile(evaluableExpr, vsumFunc)
						if err != nil {
