...
// DefaultTableParagraphTransformer is DefaultTableParagraphTransformer
var DefaultTableParagraphTransformer = &tableParagraphTransformer{}

// SegmentsMap is SegmentsMap
var SegmentsMap map[gast.Node][]text.Segment
...
func (b *tableParagraphTransformer) Transform(node *gast.Paragraph, reader text.Reader, pc parser.Context) {
	...
	for i := 1; i < lines.Len(); i++ {
		...
		if SegmentsMap == nil {
			SegmentsMap = make(map[gast.Node][]text.Segment)
		}
		table := ast.NewTable()
		table.Alignments = alignments
		table.AppendChild(table, ast.NewTableHeader(header))
		SegmentsMap[table] = append(SegmentsMap[table], lines.At(i-1))
		for j := i + 1; j < lines.Len(); j++ {
			SegmentsMap[table] = append(SegmentsMap[table], lines.At(j))
